...
type Recap = { user_id: string; display_name: string; recap_text: string };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase
        .from("weekly_recaps")
        .select("user_id, week_of, recap_text")
        .eq("group_id", groupId)
      ...
      const rows = (data ?? [])
        ...
        .map((r: any) => ({ user_id: r.user_id, recap_text: r.recap_text, display_name: nameById.get(r.user_id) ?? "Someone" }));
...
const styles = StyleSheet.create({
  ...
  name: { fontWeight: "800", fontSize: 13, color: "#0F172A", marginBottom: 2 },
