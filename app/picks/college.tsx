...
export default function PicksCollege() {
  ...
  // if you also pass ?w=7 we’ll respect it; otherwise fall back to helper
  const [week, setWeek] = useState<number>(
    typeof params.w === "string" ? Number(params.w) || 1 : getCurrentCfbWeek?.() ?? 1
  );
  ...
  const [pickedGroupId, setPickedGroupId] = useState<string | undefined>(
    undefined
  );
  ...
  // load my groups for the “Save picks to a group?” banner
  useEffect(() => {
    if (groupId) return; // already in a group context
    (async () => {
      try {
        ...
        const ids = Array.from(
          new Set((mems ?? []).map((m: any) => m.group_id))
        ).filter(Boolean);
  ...
  // sync weeks/games and previously-saved picks
  useEffect(() => {
    (async () => {
      setNotice(null);
      ...
      // load my picks for those game ids, WITH or WITHOUT a group scope
      let q = supabase
        .from("picks")
  ...
  // which markets exist in the feed (enables/disables tabs)
  ...
  const savePick = async (oddsGame: any, type: BetType, o: any) => {
    ...
    const key = `${normTeamCFB(oddsGame.away_team)}@${normTeamCFB(
      oddsGame.home_team
    )}`;
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {!data?.length ? (
        ...
        data.map((game: any) => {
          ...
          const mappedId =
            gameMap[
              `${normTeamCFB(game.away_team)}@${normTeamCFB(
                game.home_team
              )}`
            ];
          ...
          return (
            <View key={game.id} style={styles.card}>
              <View style={styles.logosRow}>
                <Image
                  source={logoSrc(game.away_team, "ncaaf")}
                  style={styles.logo}
                />
                <Text style={styles.vs}>@</Text>
                <Image
                  source={logoSrc(game.home_team, "ncaaf")}
                  style={styles.logo}
                />
              </View>
