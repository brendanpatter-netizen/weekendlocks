...
- No avatar chip in Standings or Power Rankings — the member's name is already the row's identity; a colored initials circle next to it repeated the same information. The avatar chip is kept where it's the *only* identity marker (Recent Activity, the Weekly Locks grid header, chat).
...
- **Don't** add a member avatar chip next to a name that's already shown in the same row (Standings, Power Rankings) — it's redundant identity, not a second signal. Keep the avatar where the name isn't already adjacent, or where the chip is the primary identity marker (Recent Activity, Weekly Locks grid header, chat).
