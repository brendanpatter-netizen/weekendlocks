...
**The Same Hash, Different Canvas Rule (narrowed).** The six pastel/deep identity color pairs (`lib/avatar.ts`) power identity color wherever it's shown, keyed by whichever id is being colored — member id for the Weekly Locks grid's header background and the groups list's avatar cluster, group id for the groups list row's own name text (the one place the hash colors a group rather than a member). Standings, Power Rankings, Recent Activity, and Chat intentionally show no avatar chip at all (see Key Characteristics); this rule governs the color whenever identity color does appear, not whether a chip exists. Under The Whiteboard this rule no longer extends to group hero backgrounds — every group now shares one board color, and per-group identity moved to the hand-lettered name instead, both on its own dashboard hero and, now, in the groups list. Never assign these six colors manually or add a seventh; the hash math and every existing color assignment depend on the array staying fixed.
...
### Named Rules
**The Two-Voice Rule.** PermanentMarker is reserved for exactly two roles — the group name (its own dashboard hero at 40px chalk-white, and the groups list row at 20px, identity-colored — same role, two contexts, not a third) and section titles. It never appears on a button, a data value, or body copy; introducing it elsewhere dilutes the one signal that something is "written on the board" versus "printed on paper."
...
**The Accent-Only Tilt Rule (final).** Nothing in this product tilts except `TapeCorner` (`-3deg`/`3deg`) — full stop. Three separate passes tried to keep tilt somewhere else — whole cards, the Power Rankings #1 circle, the home page's scattered team-logo badges and step-icon chips — and direct user feedback flagged every one of them as looking crooked, not hand-drawn. `TapeCorner` survives only because it reads unambiguously as a physical prop (literal masking tape), not as UI. Do not reintroduce rotation anywhere else in this codebase, including small "decorative" accents that seem safe — that reasoning has already been tried and rejected twice.
...
### Logo (`LogoMark` / `LogoBadge` / `BrandLockup`)
The brand mark joined the same drawn-icon family as the section icons — replaced a solid-white-padlock-on-teal-circle badge that was the one piece of UI still speaking a different visual language than the rest of the app. `LogoMark` reuses `LockIcon`'s exact shackle and body paths, with a football's laces (thinner-stroke oval, seam, three ticks) standing in for the keyhole — the "padlock built from a football" concept survives, just redrawn as an unfilled stroke instead of a filled cutout. `BrandLockup` (header) renders the mark bare in chalk-white beside a PermanentMarker "WeekendLocks" wordmark, exactly like any other on-board headline — no badge, no circle, no separate treatment from the rest of the board. `LogoBadge` (the standalone "app icon" moment on sign-in and reset-password, the two routes where the global header is deliberately hidden) is the one place the mark still sits inside a container: a chalk-paper chip with a dashed border, marker-red mark inside — matching every other card's material instead of a solid color-fill badge. It's always wrapped in a `Link` to `/` — the header being hidden on auth routes must never mean there's no way back to the home page.

### How It Works (rules page)
On-board hero (eyebrow + PermanentMarker title, same recipe as a group's own hero) leading into a stack of square paper cards, one per rule topic — prose kept to one short line or sentence each, with the demonstrating rather than explaining done by three static illustrative mocks: a sample game card, a sample Standings table, and a sample Weekly Locks grid (win/loss/pending cells, same colors as the real grid). All three are deliberately styled close to but distinct from their real counterparts (solid border, not dashed) so each reads as "here's roughly what that looks like," not as a live, tappable control.

### Groups List (board preview rows)
Page order is deliberately split into two moments: "Start your group" leads (26px chalk-white, the primary h1) over the Create/Join action cards, then "Your groups" (20px chalk-white, secondary — same on-board chalk-white as the primary since neither sits on paper) introduces the actual list below it. The action of starting something and the fact of what you're already in don't share one heading.

Each group row is a repeated-list item (chalk-paper, dashed border, no `TapeCorner`, no tilt — see The List Restraint Rule) but carries more than a name now: no initials badge next to the group name — the same redundant-identity call already made for Standings/Power Rankings/Recent Activity/Chat (see Key Characteristics) extended to the one remaining place it hadn't been made. In its place, the group name itself is hand-lettered (PermanentMarker, 20px, the same "Title" size as any paper card's section title) and colored per-group via the identity hash (`avatarColor(groupId).fg`, applied inline, never hardcoded) — the first and only place that hash keys off a *group* id rather than a member id, and the first place it colors text rather than an avatar chip's fill+text pair. This is a narrow, deliberate exception to "never extend the identity hash beyond its documented uses" (see The Same Hash, Different Canvas Rule) — it doesn't reintroduce per-group hero backgrounds, it just gives the group's own name the color signal the removed badge used to carry, now that nothing else on the row carries it.

A second line below the name shows a member avatar cluster (identity-colored circles, `marginLeft: -8` overlap, a 2px chalk-white border so overlapping circles read as cut out from each other rather than muddying together — capped at 4 with a `+N` overflow chip) and the group's current standings leader (`TrophyIcon` in marker-red + name + record, same win% ranking rule as the Standings card, omitted entirely rather than naming a 0-0 "leader" when nobody's decided a pick yet). When the leader has a decided record, their most recent lock's team logo sits flush right on that same line in a small white-backed circle (`leaderLine` is `flex: 1` specifically so the logo's `marginLeft: "auto"` reaches the row's true right edge, not just the text's) — the one place a real photographic team logo appears outside a picks page. A `"Picks open"` callout — chalk-paper, dashed marker-red border, square, absolutely positioned to overlap the row's top-right corner — appears only when the viewer specifically hasn't made this week's required picks in that group (respecting the CFB gap-week rule: two CFB locks instead of one CFB + one NFL). This is the one place in the product a list row carries a second "preview" line instead of being a pure link — it exists so the groups list itself has a reason to glance at, not just route through.
...
### Navigation
Top bar is the logo, centered, plus a single hamburger button (`Ionicons` `menu-outline`/`close`) pinned to the right, on the board-felt background — every nav link that used to sit inline in the header now lives in one dropdown, opened from that button. The logo sits in an absolutely-positioned centering wrapper (`pointerEvents: "box-none"`) so the corner button doesn't visually or functionally compete with it. The dropdown itself is a chalk-paper, dashed-border, square panel (see The Accent-Only Tilt Rule — it's UI, not a prop) anchored under the button, with a `card-lift`-strength shadow so it reads as floating above the page; a full-screen transparent overlay behind it closes the menu on outside tap, and it also auto-closes on every navigation. Order is fixed: **How It Works, Groups, Account, Sign out** (or Sign in, signed out) — How It Works leads because it's the one entry a brand-new, not-yet-signed-in visitor needs first.
...
- **Don't** add an avatar/initials chip next to a name that's already shown in the same row — it's redundant identity, not a second signal. This applies to member identity everywhere in the product (Standings, Power Rankings, Recent Activity, Chat all dropped theirs) and to the groups list's own group-initial badge; only add identity color where the name isn't separately visible (the Weekly Locks grid header), or *as* the name's own color when nothing else on the row still carries the signal (the groups list row).
